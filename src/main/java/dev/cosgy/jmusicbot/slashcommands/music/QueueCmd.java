...
public class QueueCmd extends MusicCommand {
    ...
    public QueueCmd(Bot bot) {
        super(bot);
        this.name = "queue";
        this.help = "Displays the list of songs in the play queue";
        this.arguments = "[page]";
    ...
    public void doCommand(CommandEvent event) {
        ...
        if (list.isEmpty()) {
            ...
            MessageCreateData built = new MessageCreateBuilder()
                    .setContent(event.getClient().getWarning() + " There are no songs in the play queue.")
    ...
    public void doCommand(SlashCommandEvent event) {
        InteractionHook m = event.reply("Fetching queue...").complete();
        int pagenum = 1;
        AudioHandler ah = (AudioHandler) event.getGuild().getAudioManager().getSendingHandler();
        List<QueuedTrack> list = ah.getQueue().getList();
        if (list.isEmpty()) {
    ...
    private String getQueueTitle(AudioHandler ah, String success, int songslength, long total, RepeatMode repeatmode) {
        ...
        return FormatUtil.filter(sb.append(success).append(" Playlist Queue | ").append(songslength)
