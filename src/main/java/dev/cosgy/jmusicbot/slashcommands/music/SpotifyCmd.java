...
public class SpotifyCmd extends MusicCommand {
    ...
    public SpotifyCmd(Bot bot) {
        ...
        options.add(new OptionData(OptionType.STRING, "tracklink", "Spotify track URL", true));
    ...
    public void doCommand(SlashCommandEvent event) {
        ...
        if(accessToken == null){
            event.reply("This command is unavailable. This command requires settings from the bot owner.").queue();
            return;
        }

        // If the access token is expired, reissue it
    ...
    public void doCommand(CommandEvent event) {
        ...
        if(accessToken == null){
            event.reply("This command is unavailable. To enable this command, a setting from the bot owner is required.");
            return;
        }
    ...
    private class SlashResultHandler implements AudioLoadResultHandler {
        ...
        public void trackLoaded(AudioTrack track) {
            ...
            event.getHook().sendMessage(FormatUtil.filter(event.getClient().getWarning() + "**" + track.getInfo().title + "** is too long."
            ...
            event.getHook().sendMessage(FormatUtil.filter(event.getClient().getSuccess() + "**" + track.getInfo().title
                    ...
                    : "has been added at position " + pos + " of the playlist queue."))).queue();
        }
        ...
        public void playlistLoaded(AudioPlaylist playlist) {
            builder.setColor(event.getGuild().getSelfMember().getColor())
                    ...
                    .setSelection((msg, i) ->
                    {
                        ...
                        if (bot.getConfig().isTooLong(track)) {
                            event.getHook().sendMessage(event.getClient().getWarning() + "**" + track.getInfo().title + "** is too long."
                                    + FormatUtil.formatTime(track.getDuration()) + "` > `" + bot.getConfig().getMaxTime() + "`").queue();
                            return;
                        }
                        AudioHandler handler = (AudioHandler) event.getGuild().getAudioManager().getSendingHandler();
                        ...
                        event.getHook().sendMessage(event.getClient().getSuccess() + "**" + track.getInfo().title
                                + "**(`" + FormatUtil.formatTime(track.getDuration()) + "`) " + (pos == 0 ? "has been added."
                                : "has been added at position " + pos + " of the playlist queue."))
                                .queue();
                    })
                    .setCancel((msg) -> {
                    })
        ...
        public void noMatches() {
            event.getHook().sendMessage(FormatUtil.filter(event.getClient().getWarning() + " No matches found for the search. `")).queue();
        }
        ...
        public void loadFailed(FriendlyException throwable) {
            ...
            if (throwable.severity == FriendlyException.Severity.COMMON)
                event.getHook().sendMessage(event.getClient().getError() + " Error occurred while loading: " + throwable.getMessage()).queue();
            else
                event.getHook().sendMessage(event.getClient().getError() + " Error occurred while loading").queue();
        }
    }
    ...
    private class ResultHandler implements AudioLoadResultHandler {
        ...
        public void trackLoaded(AudioTrack track) {
            if (bot.getConfig().isTooLong(track)) {
            m.editMessage(FormatUtil.filter(event.getClient().getWarning() + "**" + track.getInfo().title + "** is too long. "
                    + FormatUtil.formatTime(track.getDuration()) + "` > `" + bot.getConfig().getMaxTime() + "`")).queue();
            return;
        }
        AudioHandler handler = (AudioHandler) event.getGuild().getAudioManager().getSendingHandler();
        ...
        public void playlistLoaded(AudioPlaylist playlist) {
            builder.setColor(event.getSelfMember().getColor())
                    ...
                    event.replySuccess("**" + FormatUtil.filter(track.getInfo().title)
                            + "** (`" + FormatUtil.formatTime(track.getDuration()) + "`) " + (pos == 0 ? "has been added to the queue to start playing."
                            : "has been added at position " + pos + " of the playlist queue."));
                    })
                    .setCancel((msg) -> {
                    })
        ...
        @Override
        public void noMatches() {
            m.editMessage(FormatUtil.filter(event.getClient().getWarning() + " No matches were found for the search. `")).queue();
        }
        ...
        public void loadFailed(FriendlyException throwable) {
            ...
            if (throwable.severity == FriendlyException.Severity.COMMON)
                ...
                m.editMessage(event.getClient().getError() + " Error occurred while loading").queue();
        }
    }
}
