...
public class SearchCmd extends MusicCommand {
    ...
    public SearchCmd(Bot bot) {
        ...
        this.help = "Searches YouTube using the specified string.";
    ...
    private class SlashResultHandler implements AudioLoadResultHandler {
        ...
        public void trackLoaded(AudioTrack track) {
            if (bot.getConfig().isTooLong(track)) {
                m.editOriginal(FormatUtil.filter(event.getClient().getWarning() + "**" + track.getInfo().title + "** is longer than the maximum permitted duration:"
        ...
        public void playlistLoaded(AudioPlaylist playlist) {
            builder.setColor(event.getGuild().getSelfMember().getColor())
                    .setText(FormatUtil.filter(event.getClient().getSuccess() + " `" + event.getOption("input").getAsString() + "` search results:"))
                    .setChoices()
                    .setSelection((msg, i) ->
                    {
                        AudioTrack track = playlist.getTracks().get(i - 1);
                        if (bot.getConfig().isTooLong(track)) {
                            event.reply(event.getClient().getWarning() + "**" + track.getInfo().title + "** is longer than the maximum permitted duration:"
                                    + FormatUtil.formatTime(track.getDuration()) + "` > `" + bot.getConfig().getMaxTime() + "`").queue();
                            return;
                        }
                        AudioHandler handler = (AudioHandler) event.getGuild().getAudioManager().getSendingHandler();
            ...
            ;
            for (int i = 0; i < 8 && i < playlist.getTracks().size(); i++) {
        ...
        public void loadFailed(FriendlyException throwable) {
            ...
            if (throwable.severity == Severity.COMMON)
                ...
                m.editOriginal(event.getClient().getError() + " Error occurred while loading").queue();
        }
    }
    ...
    private class ResultHandler implements AudioLoadResultHandler {
        ...
        public void trackLoaded(AudioTrack track) {
            if (bot.getConfig().isTooLong(track)) {
            m.editMessage(FormatUtil.filter(event.getClient().getWarning() + " This track (**" + track.getInfo().title + "**) is longer than the permitted maximum length: `"
                    + FormatUtil.formatTime(track.getDuration()) + "` > `" + bot.getConfig().getMaxTime() + "`")).queue();
            return;
        }
        AudioHandler handler = (AudioHandler) event.getGuild().getAudioManager().getSendingHandler();
        ...
        m.editMessage(FormatUtil.filter(event.getClient().getSuccess() + " **" + track.getInfo().title
                + "** (`" + FormatUtil.formatTime(track.getDuration()) + "`) " + (pos == 0 ? "has been added to start playing."
                : "has been added to position " + pos + " in the queue."))).queue();
        }
        ...
        public void playlistLoaded(AudioPlaylist playlist) {
            builder.setColor(event.getSelfMember().getColor())
                    ...
                    .setSelection((msg, i) ->
                    {
                        AudioTrack track = playlist.getTracks().get(i - 1);
                        if (bot.getConfig().isTooLong(track)) {
                            event.replyWarning("This track (**" + track.getInfo().title + "**) is longer than the permitted maximum length: `"
                        ...
                        event.replySuccess("**" + FormatUtil.filter(track.getInfo().title)
                                ...
                                : "has been added to position " + pos + " in the queue."));
                    })
                    .setCancel((msg) -> {
                    })
        ...
        @Override
        public void noMatches() {
            m.editMessage(FormatUtil.filter(event.getClient().getWarning() + " No matches found for `" + event.getArgs() + "`")).queue();
        }
