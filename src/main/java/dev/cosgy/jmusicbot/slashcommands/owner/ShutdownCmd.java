...
public class ShutdownCmd extends OwnerCommand {
    ...
    protected void execute(SlashCommandEvent event) {
        event.reply(event.getClient().getWarning() + "Shutting down...\nThere may be issues with a proper shutdown. If necessary, please force the bot to shut down.").queue();
        bot.shutdown();
    }

    @Override
    protected void execute(CommandEvent event) {
        event.replyWarning("Shutting down...\nThere may be issues with a proper shutdown. If necessary, please force the bot to shut down.");
        bot.shutdown();
    }
}
