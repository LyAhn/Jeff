...
public class SetstatusCmd extends OwnerCommand {
    ...
    protected void execute(SlashCommandEvent event) {
        try {
            ...
            if (status == OnlineStatus.UNKNOWN) {
                event.reply(event.getClient().getError() + "Include one of the following statuses: `ONLINE`, `IDLE`, `DND`, `INVISIBLE`").queue();
            } else {
                event.getJDA().getPresence().setStatus(status);
                event.reply(event.getClient().getSuccess() + "Set status to `" + status.getKey().toUpperCase() + "`").queue();
            }
        } catch (Exception e) {
    ...
    protected void execute(CommandEvent event) {
        try {
            ...
            if (status == OnlineStatus.UNKNOWN) {
                event.replyError("Include one of the following statuses: `ONLINE`, `IDLE`, `DND`, `INVISIBLE`");
            } else {
                event.getJDA().getPresence().setStatus(status);
                event.replySuccess("Set status to `" + status.getKey().toUpperCase() + "`");
            }
        } catch (Exception e) {
