...
public class SetgameCmd extends OwnerCommand {
    ...
    protected void execute(CommandEvent event) {
        ...
        try {
            ...
            event.reply(event.getClient().getSuccess() + " **" + event.getSelfUser().getName()
                    + "** is now " + (title.isEmpty() ? "back to nothing." : "currently playing `" + title + "`"));
        } catch (Exception e) {
            event.reply(event.getClient().getError() + " I was unable to set the status.");
        }
    }
    ...
    private class NoneCmd extends OwnerCommand {
        ...
        protected void execute(SlashCommandEvent event) {
            ...
            event.reply("Reset status.").queue();
        }

        @Override
        protected void execute(CommandEvent event) {
            event.getJDA().getPresence().setActivity(null);
            event.reply("Reset status.");
        }
    }
    ...
    private class SetstreamCmd extends OwnerCommand {
        private SetstreamCmd() {
            this.name = "stream";
            this.aliases = new String[]{"twitch", "streaming"};
            this.help = "Sets the game the bot is playing as a streaming status.";
        ...
        protected void execute(CommandEvent event) {
            ...
            if (parts.length < 2) {
                event.replyError("Please provide a 'streaming username' and a 'game name'");
                return;
            }
            try {
                ...
                event.replySuccess("**" + event.getSelfUser().getName()
                        + "** is currently listening to `" + parts[1] + "`.");
            } catch (Exception e) {
                event.reply(event.getClient().getError() + "Failed to set game.");
            }
        }
    }
    ...
    private class SetlistenCmd extends OwnerCommand {
        private SetlistenCmd() {
            ...
            this.help = "Sets the game that the bot is listening to";
            this.arguments = "<title>";
            this.guildOnly = false;
            List<OptionData> options = new ArrayList<>();
        ...
        protected void execute(CommandEvent event) {
            if (event.getArgs().isEmpty()) {
                event.replyError("Please include the title you want to listen to.");
                return;
            }
    ...
    private class SetwatchCmd extends OwnerCommand {
        private SetwatchCmd() {
            ...
            this.help = "Sets the game that the bot is watching";
            this.arguments = "<title>";
            this.guildOnly = false;
            List<OptionData> options = new ArrayList<>();
            options.add(new OptionData(OptionType.STRING, "title", "Title", true));
        ...
        protected void execute(SlashCommandEvent event) {
            ...
            try {
                ...
                event.reply(event.getClient().getSuccess() + "**" + event.getJDA().getSelfUser().getName() + "** is currently watching `" + title + "`.").queue();
            } catch (Exception e) {
                event.reply(event.getClient().getError() + " Unable to set the game.").queue();
            }
        }
        ...
        protected void execute(CommandEvent event) {
            if (event.getArgs().isEmpty()) {
                event.replyError("Please enter the title you want to be watching.");
                return;
            }
            ...
            try {
                ...
                event.reply(event.getClient().getError() + " Unable to set the game.");
            }
        }
    }
    ...
    private class SetCompetingCmd extends OwnerCommand {
        private SetCompetingCmd() {
            this.name = "competing";
            this.help = "Sets the game that the bot is competing in";
            this.arguments = "<title>";
            this.guildOnly = false;
            List<OptionData> options = new ArrayList<>();
            options.add(new OptionData(OptionType.STRING, "title", "Game title", true));
        ...
        protected void execute(SlashCommandEvent event) {
            ...
            try {
                ...
                event.reply(event.getClient().getSuccess() + "**" + event.getJDA().getSelfUser().getName() + "** is now competing at `" + title + "`.").queue();
            } catch (Exception e) {
                event.reply(event.getClient().getError() + " Unable to set the game.").queue();
            }
        }
        ...
        protected void execute(CommandEvent event) {
            if (event.getArgs().isEmpty()) {
                event.replyError("Please enter the competing title.");
                return;
            }
            String title = event.getArgs();
            try {
                event.getJDA().getPresence().setActivity(Activity.watching(title));
                event.replySuccess("**" + event.getSelfUser().getName() + "** is currently competing in `" + title + "`.");
            } catch (Exception e) {
                event.reply(event.getClient().getError() + " Unable to set the game.");
            }
        }
