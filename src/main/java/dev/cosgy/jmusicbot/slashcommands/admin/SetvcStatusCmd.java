...
public class SetvcStatusCmd extends AdminCommand {
    public SetvcStatusCmd(Bot bot) {
        ...
        this.help = "Sets whether to display playing status in VC status.";
    ...
    protected void execute(SlashCommandEvent event) {
        ...
        event.reply(event.getClient().getSuccess() + "Set whether to display playing status in VC status to `" + status + "`.").queue();
    }
    ...
    protected void execute(CommandEvent event) {
        ...
        if (event.getArgs().toLowerCase().matches("(false|disabled)")) {
            s.setVCStatus(false);
            event.reply(event.getClient().getSuccess() + "Set not to display playing status in VC status.");
        }else if(event.getArgs().toLowerCase().matches("(true|enabled)")) {
            s.setVCStatus(true);
            event.reply(event.getClient().getSuccess() + "Set to display playing status in VC status.");
        }else {
            event.reply(event.getClient().getError() + "Please include true or false.");
        }
    }
