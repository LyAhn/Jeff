...
public class HelpCmd extends SlashCommand {
    ...
    public HelpCmd(Bot bot) {
        this.bot = bot;
        this.name = "help";
        this.help = "Displays the command list.";
    ...
    protected void execute(SlashCommandEvent event) {
        ...
        if (event.getClient().getServerInvite() != null)
            builder.append("\n\nYou can also join the official server for more help: ").append(event.getClient().getServerInvite());
    ...
    public void execute(CommandEvent event) {
        ...
        if (bot.getConfig().getHelpToDm()) {
            event.replyInDm(builder.toString(), unused ->
            {
                if (event.isFromType(ChannelType.TEXT))
                    event.reactSuccess();
            }, t -> event.replyWarning("Direct messages are blocked, so help cannot be sent."));
        } else {
