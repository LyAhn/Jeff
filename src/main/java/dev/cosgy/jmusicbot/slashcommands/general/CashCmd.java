...
public class CashCmd extends SlashCommand {
    ...
    protected void execute(CommandEvent event) {
        if (!bot.getCacheLoader().cacheExists(event.getGuild().getId())) {
            event.reply("There were no songs cached.");
            return;
        }
        int pagenum = 1;
        try {
    ...
    private String getQueueTitle(String success, int songsLength, long total) {
        ...
        return FormatUtil.filter(sb.append(success).append(" List of cached songs | ").append(songsLength)
    ...
    public static class DeleteCmd extends DJCommand {
        public DeleteCmd(Bot bot) {
            ...
            this.help = "Deletes the cached songs.";
            this.guildOnly = true;
        }
        ...
        public void doCommand(CommandEvent event) {
            if (!bot.getCacheLoader().cacheExists(event.getGuild().getId())) {
                event.reply("There is no cache to delete.");
                return;
            }
            ...
            try {
                ...
                event.reply("Error occurred while deleting cache.");
                e.printStackTrace();
                return;
            }
            event.reply("Cache deleted.");
        }
        ...
        public void doCommand(SlashCommandEvent event) {
            ...
            try {
                ...
                event.reply("An error occurred while trying to delete the cache.").queue();
                e.printStackTrace();
                return;
            }
            event.reply("Deleted cache.").queue();
        }
    }
    ...
    public class ShowCmd extends SlashCommand {
        ...
        public ShowCmd(Bot bot) {
            this.name = "show";
            this.help = "Displays the songs cached.";
            this.guildOnly = true;
            this.botPermissions = new Permission[]{Permission.MESSAGE_ADD_REACTION, Permission.MESSAGE_EMBED_LINKS};
            builder = new Paginator.Builder()
                    .setColumns(1)
        ...
        protected void execute(SlashCommandEvent event) {
            if (!bot.getCacheLoader().cacheExists(event.getGuild().getId())) {
                event.reply("There were no songs cached.").queue();
                return;
            }
            int pagenum = 1;
