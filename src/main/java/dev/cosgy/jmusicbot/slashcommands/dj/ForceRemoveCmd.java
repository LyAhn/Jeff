...
public class ForceRemoveCmd extends DJCommand {
    ...
    public void doCommand(SlashCommandEvent event) {
        ...
        AudioHandler handler = (AudioHandler) event.getGuild().getAudioManager().getSendingHandler();
        if (handler.getQueue().isEmpty()) {
            event.reply(event.getClient().getError() + "The playlist is empty!").queue();
        }

        User target = event.getOption("user").getAsUser();
        int count = ((AudioHandler) event.getGuild().getAudioManager().getSendingHandler()).getQueue().removeAll(target.getIdLong());
        if (count == 0) {
            event.reply(event.getClient().getWarning() + "There were no songs in **" + target.getName() + "**'s playlist!").queue();
        } else {
            event.reply(event.getClient().getSuccess() + "Removed `" + count + "` songs from **" + target.getName() + "**#" + target.getDiscriminator() + "'s playlist.").queue();
        }
    }
    ...
    private void removeAllEntries(User target, CommandEvent event) {
        int count = ((AudioHandler) event.getGuild().getAudioManager().getSendingHandler()).getQueue().removeAll(target.getIdLong());
        if (count == 0) {
            event.replyWarning("There were no songs in **" + target.getName() + "'s playlist!");
        } else {
            event.replySuccess("Removed `" + count + "` songs from **" + target.getName() + "**#" + target.getDiscriminator() + "'s playlist.");
        }
    }
}